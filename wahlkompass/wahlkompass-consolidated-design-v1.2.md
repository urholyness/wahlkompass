# Wahlkompass — Consolidated Technical Design (v1.2)
...
**Provenance:** v1.0 architecture + v1.1 amendments + v1.2 revisions from adversarial cross-model review (all §16 questions answered; accepted resolutions folded in below, each marked *v1.2*). No external references required.
...
Define the position-uncertainty radius rᵢ = (1 − cᵢ). Two intervals, two jobs (*v1.2*):

```
Display half-width (heuristic standard error, independent-error assumption, z = 1):

    h(P) = √( Σ_{i∈A} wᵢ²·rᵢ² )  /  ( 2 · Σ_{i∈A} wᵢ )

Tie-grouping bound (conservative, fully-correlated worst case):

    h_max(P) = Σ_{i∈A} wᵢ·rᵢ  /  ( 2 · Σ_{i∈A} wᵢ )
```

The display interval is readable and one-sentence explainable ("Fehlerspanne = Standardabweichung des Übereinstimmungswertes bei Annahme unabhängiger Unsicherheiten"); the Methodik page states its known limitation in the next sentence — evidence errors share systematic sources (coder tendencies, tier calibration), so true uncertainty lies between h and h_max. The **conservative bound governs rank claims**: two parties are displayed as tied whenever their scores differ by less than max(h_max(P₁), h_max(P₂)). Honest visuals, conservative claims. Display: **"Übereinstimmung: 71 % ± 4"**; ties render as shared rank — arbitrary tie-breaking between parties is a bias vector.
...
Direction coding per item — roll-call votes map mechanically (*v1.2, replacing the earlier Enthaltung-as-0 rule*): with Fraktion counts (Y, N, E) among members present,

```
direction     x = (Y − N) / (Y + N)                    (active votes only)
item weight   = (Y + N) / (Y + N + E)                  (participation share)
Unified abstention: if (Y+N)/(Y+N+E) < 0.10 → item excluded from pᵢ entirely,
logged in the evidence drawer as "geschlossene Enthaltung" (strategic abstention
is coalition discipline, not neutrality — scoring it as 0 was a category error).
```

Additionally, every T1 item carries a display-only **Fraktion Cohesion Index** FCI = max(Y,N,E)/(Y+N+E); if FCI < 0.90 the evidence card shows a split-vote note ("Die Fraktion stimmte uneinheitlich ab: X % Ja, Y % Nein"). Mechanical, identical for every party, D6-compliant — intra-party division is displayed, never scored. T2/T3/T4 items get a direction assigned by a **paired-reviewer process**: two reviewers code independently on the 5-point scale; disagreement > 0.5 escalates to the methodology board (§3.6). Both codings and the resolution are stored.
...
```
volume    v = 1 − exp(−W / W₀),      W = Σₑ τₑ·ρ(e)
          W₀ = −N* / ln(1 − η)       (v1.2: policy form)
          Board-set policy: N* = 3 fresh T1 items ⇒ confidence target η = 0.90
          ⇒ W₀ ≈ 1.30
agreement a = 1 − σ_w(x) / σ_max,    σ_w = weighted std of item directions, σ_max = 1
cᵢ = v · a
```

*v1.2 note on constants.* The policy form does not remove the free parameter — it relocates it from an opaque W₀ into (N*, η), a sentence the board can debate and the Methodik page can print: "drei aktuelle namentliche Abstimmungen ergeben 90 % Konfidenz." λ was already principled in v1.0 (half-life = one legislative period). The τ tiers remain asserted in v1; a Phase-5 calibration study will estimate empirical channel reliability (τ_k = 1 − ε_k, where ε_k = historical rate at which tier-k items contradict subsequent T1 votes on the same statement) — **pooled across parties only**: per-party τ would encode "this party's manifestos are less trustworthy," violating identical rules regardless of what the data shows. Until that study has statistical power, asserted-and-published beats estimated-and-unstable.
...
Ω(C)    = 1 − Σ_{i∈A_C} wᵢ · σ_{C_i}(i) / Σ_{i∈A_C} wᵢ           (cohesion on the user's issues)
```

*v1.2 — missing-cell handling.* Let C_i ⊆ C be the members with a non-null position on statement i. If |C_i| ≤ 1 the statement is excluded from Ω (a lone position is not cohesion); if |C_i| ≥ 2, σ is computed with seat weights renormalized over C_i (s′_P = s_P / Σ_{Q∈C_i} s_Q), and A_C = A ∩ {i : |C_i| ≥ 2}. The UI shows Ω's coverage explicitly: "Kohäsion basiert auf X von Y Ihrer Fragen." Normalization by σ_max = 1 is exact — the seat-weighted std on [−1,1] is maximized by a 50/50 boundary split (σ = 2√(f(1−f)), max 1.0 at f = ½); a corollary worth documenting is that lopsided coalitions cannot reach σ = 1, so global normalization mildly flatters unequal coalitions — accepted for cross-coalition comparability, stated on the Methodik page.
...
*v1.2 — signing-chain hardening.* In-browser verification is honest UX assurance, not the security anchor: a compromised origin serves a tampered verifier alongside a tampered bundle. The anchor is external:

1. **Transparency log.** Every release tag and bundle hash is published to Sigstore/Rekor at build time; signatures become publicly non-repudiable and split-view attacks (different bundles to different users) become detectable.
2. **Independent release monitor.** A watcher on separate infrastructure (partner university and/or a scheduled GitHub Action outside the production org) fetches the production bundle, verifies it against Rekor and the repo tag, and alerts publicly on mismatch. Cheap, and it converts "trust us" into "catch us."
3. **Out-of-band verification CLI.** `reproduce.py` doubles as the journalist/researcher tool: verify signature, check Rekor inclusion, rebuild positions.json — all off-browser.
4. **Threshold signing (Phase 0 decision).** Optionally 2-of-3 release keys (operator, board chair, independent academic partner) so no single key compromise or insider can cut a valid release; adopted if the key ceremony is operationally sustainable. The monitor and log ship regardless.

The frontend still verifies in-browser and pins the release tag in the UI footer ("Datenstand: 2026.09.1"). Anyone can: reproduce positions.json from evidence.json using the published formulas (a `reproduce.py` ships in the repo); diff releases; or run the frontend against an old release. Silent revision is structurally impossible, which is a stronger claim than "we have an audit log."
...
7. **Release-monitor liveness (v1.2).** CI asserts the external monitor saw and verified the previous release within its SLA; a dead watchdog blocks the next release.
8. **Divergence-symmetry audit.** Distribution of divergence flags across parties, published. The rule is mechanical; the audit proves it.
...
2. **Bill↔statement linking is a judgment call.** Dual-coded and published; *v1.2 upgrade:* every link ships as a **mapping dossier** — a markdown justification in the public repo, open to issues and pull requests. PRs are structured input to the board, never a vote (open review without brigading-by-merge). Still judgment, now judgment with a public paper trail and a challenge channel.
3. **New parties are structurally under-covered.** By design (no invented positions). *v1.2 upgrade — aggregated inquiry mechanism:* each "keine belegbare Position" cell carries a counter ("1.243 Nutzer möchten diese Position wissen"); once per release cycle the platform sends **one** board-verified structured inquiry per statement to the official party channel, and an authenticated official response is ingested as T2. Deliberately *not* user-triggered mail — that design is a harassment cannon aimed at small parties. The counter creates the incentive; the board channel keeps intake verifiable.
4. **Positions are Fraktion-level.** Gewissensfrage votes remain excluded from T1 (no whipped line exists to measure). *v1.2 upgrade:* ordinary split votes are no longer averaged away — the split-vote direction formula (§3.2) scores active votes mechanically, and the FCI display surfaces the division on the evidence card. Dissent is now visible everywhere it occurs, scored nowhere it should not be.
...
## 16. Review Questions — Asked and Answered (v1.2 log)

*Status: all seven questions were answered by external cross-model review (July 2026). Resolutions folded into §2.3, §3.2, §3.4, §4.3, §7, §9, §13 above, each marked v1.2. Kept below as the record of what was asked. Notable partial rejections: empirical τ deferred to a Phase-5 pooled study (per-party τ would violate identical rules), the CI independence assumption bounded rather than adopted wholesale (systematic error sources are shared), and the party-inquiry feature redesigned from user-triggered mail to one aggregated board-verified inquiry per cycle.*
...
**v1.2 changelog:** CI split into display (RSS, z=1) and tie-grouping (conservative) intervals · Enthaltung recoded — split-vote direction (Y−N)/(Y+N), participation-share item weight, unified abstentions excluded and labeled · FCI split-vote display (threshold 0.90, published) · W₀ restated in policy form (N*=3, η=0.90 ⇒ W₀≈1.30) · τ calibration study scheduled Phase 5, pooled-only · Ω missing-cell renormalization + coverage display · Rekor transparency log + independent release monitor + verification CLI + optional 2-of-3 threshold signing · monitor liveness added to CI suite · mapping dossiers (public Git, PR-as-input) · aggregated party-inquiry mechanism · §13 weaknesses 2/3/4 upgraded.

*End of consolidated design v1.2.*
